...
def enrich_with_d3fend(technique_ids):
    ...
    for tech_id in technique_ids:
        try:
            lookup_id = tech_id.split('.')[0] if '.' in tech_id else tech_id
            ...
            if response.status_code == 200:
                ...
                if bindings:
                    ...
                    for b in bindings:
                        ...
                        def_tactic = b.get('def_tactic_label', {}).get('value', '')
                        def_id = b.get('def_tech_id', {}).get('value', '')

                        if def_tech and def_tech not in seen:
                            seen.add(def_tech)
                            countermeasures.append({
                                "defensive_technique": def_tech,
                                "defensive_tactic": def_tactic,
                                "d3fend_id": def_id
                            })

                    d3fend_results[tech_id] = {
                        "countermeasures": countermeasures[:5],
                        "total_countermeasures": len(countermeasures),
                        "url": f"https://d3fend.mitre.org/offensive-technique/attack/{lookup_id}/"
                    }
                    print(f"  {tech_id}: {len(countermeasures)} countermeasures found")
                else:
...
def save_report(triage_result, groups, mitre_enrichment, d3fend_enrichment):
    ...
    output_path = "D:\\ZeroMagenta-Labs\\Reports\\triage_v5_report.json"
...
def main():
    print("=" * 60)
    print("ZEROMAGNETA AI TRIAGE TOOL v5")
   ...
   # Enrich with MITRE ATT&CK
    ...
    for tech_id, details in d3fend_enrichment.items():
        if "error" not in details:
            print(f"\n{tech_id} — {details['total_countermeasures']} total countermeasures available")
            for cm in details['countermeasures']:
                print(f"  [{cm['defensive_tactic'].upper()}] {cm['defensive_technique']}")
            print(f"  Full details: {details['url']}")
